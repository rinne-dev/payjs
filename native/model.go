...
type Request struct {
	...
	Attach     string `json:"attach"`       // N 用户自定义数据，在 notify 的时候会原样返回
	NotifyURL  string `json:"notify_url"`   // N 接收异步通知的回调地址
	Type       string `json:"type"`         // N 支付宝交易传值：alipay，微信支付无需此字段
	NoCredit   string `json:"no_credit"`    // N 禁止使用信用卡支付请传值：1
	TimeExpire string `json:"time_expire"`  // N 订单有效期。格式：20200826080808
}
...
type Response struct {
	...
	QRCode     string `json:"qr_code"`        // Y 二维码图片地址
}
